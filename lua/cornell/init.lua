...
function M.write_back()
	if not state.active or not valid_buf(state.source_buf) then
		return
	end
	if not valid_buf(state.cues_buf) or not valid_buf(state.notes_buf) or not valid_buf(state.summary_buf) then
		return
	end
...
function M.save()
	...
	if valid_buf(state.source_buf) then
		...
		if ok then
			...
			notify("Không thể ghi Cornell về Markdown.", vim.log.levels.ERROR)
		end
	end
end

function M.add_question()
	if not state.active or not valid_buf(state.cues_buf) then
		return
	end
...
function M.check()
	...
	if #duplicate > 0 then
		parts[#parts + 1] = "duplicate: " .. table.concat(duplicate, ", ")
	end
	if #orphan > 0 then
		parts[#parts + 1] = "orphan notes: " .. table.concat(orphan, ", ")
	end
...
function M.close()
	...
	if valid_win(state.review_win) and state.review_mode then
		review.leave()
	end
	...
	if valid_buf(state.cues_buf) then
		pcall(api.nvim_buf_delete, state.cues_buf, { force = true })
	end
	...
	if valid_buf(state.summary_buf) then
		pcall(api.nvim_buf_delete, state.summary_buf, { force = true })
	end
	if valid_buf(state.review_buf) then
		pcall(api.nvim_buf_delete, state.review_buf, { force = true })
	end
...
function M.open()
	...
	if not layout.open(parsed) then
		notify("Không thể tạo Cornell layout.", vim.log.levels.ERROR)
		return
	end
	...
	if valid_win(state.cues_win) then
		pcall(api.nvim_win_set_cursor, state.cues_win, { 1, 0 })
	end
